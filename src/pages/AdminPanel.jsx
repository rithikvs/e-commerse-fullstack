...
function AdminPanel() {
  ...
  const syncAllData = async () => {
    try {
      const response = await fetch('https://e-commerse-fullstack-1.onrender.com/api/auth/sync/all', {
  ...
  const verifyAdmin = async () => {
    try {
      ...
      const response = await fetch('https://e-commerse-fullstack-1.onrender.com/api/auth/admin/verify', {
        method: 'POST',
        headers: {
  ...
  const downloadReport = async () => {
    try {
      const response = await fetch('https://e-commerse-fullstack-1.onrender.com/api/auth/generate-report', {
        headers: { 'x-admin-key': getAdminKey() }
      });
...
`;
  ...
  const handleDeleteUser = async (email) => {
    try {
      const response = await fetch(`https://e-commerse-fullstack-1.onrender.com/api/auth/users/${email}`, {
        method: 'DELETE',
        headers: { 'x-admin-key': getAdminKey() }
      });
  ...
  const handleDeleteProduct = async (productId) => {
    try {
      const response = await fetch(`https://e-commerse-fullstack-1.onrender.com/api/products/${productId}/admin`, {
        method: 'DELETE',
        headers: { 'x-admin-key': getAdminKey() }
      });
  ...
  const handleDeleteCart = async (userEmail) => {
    try {
      const response = await fetch(`https://e-commerse-fullstack-1.onrender.com/api/cart/${userEmail}`, {
        method: 'DELETE',
        headers: { 'x-admin-key': getAdminKey() }
      });
  ...
  const saveStock = async (productId) => {
    try {
      const response = await fetch(`https://e-commerse-fullstack-1.onrender.com/api/products/${productId}/admin/stock`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleApproveProduct = async (productId) => {
    try {
      const response = await fetch(`https://e-commerse-fullstack-1.onrender.com/api/products/${productId}/approve`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'x-admin-key': getAdminKey()
  ...
  const handleRejectProduct = async (productId) => {
    try {
      const response = await fetch(`https://e-commerse-fullstack-1.onrender.com/api/products/${productId}/reject`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'x-admin-key': getAdminKey()
  ...
  const fetchOrders = async (suppressLoading = false) => {
    ...
    try {
      ...
      try {
        const res = await fetch('https://e-commerse-fullstack-1.onrender.com/api/orders/all', {
      ...
      if (!data || data.length === 0) {
        ...
        for (const u of usersToQuery) {
          ...
          try {
            const r = await fetch(`https://e-commerse-fullstack-1.onrender.com/api/orders/user/${encodeURIComponent(u.email)}`);
            if (r.ok) {
        ...
        try {
          const cu = JSON.parse(localStorage.getItem('currentUser') || '{}');
          if (cu?.email) {
            const r2 = await fetch(`https://e-commerse-fullstack-1.onrender.com/api/orders/user/${encodeURIComponent(cu.email)}`);
            if (r2.ok) {
  ...
  // Whenever the main orders list changes we clear any previous date-filter results
  useEffect(() => {
    setOrdersByDate([]);
  }, [orders]);

  // Filter orders by selectedDate (YYYY-MM-DD)
  const filterOrdersByDate = () => {
    if (!selectedDate) {
      alert('Please pick a date to filter orders.');
      return;
    }
    const filtered = (orders || []).filter(o => {
      const d = o?.createdAt ? new Date(o.createdAt).toISOString().slice(10) : '';
      return d === selectedDate;
    });
    setOrdersByDate(filtered);
  };
  ...
  const handleDownloadCSV = async () => {
    ...
    try {
      ...
      const res = await fetch('https://e-commerse-fullstack-1.onrender.com/api/orders/export', {
      ...
      <section style={styles.section}>
        ...
        <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <label style={{ fontSize: 13, color: '#333', display: 'flex', alignItems: 'center', gap: 8 }}>
              Date:
            ...
            <button onClick={filterOrdersByDate} style={{ padding: '8px 12px', borderRadius: 8, background: '#27ae60', color: '#fff', border: 'none', cursor: 'pointer' }}>Filter</button>
            <button onClick={clearDateFilter} style={{ padding: '8px 12px', borderRadius: 8, background: '#95a5a6', color: '#fff', border: 'none', cursor: 'pointer' }}>Clear</button>
            <button
        ...
        {/* If a date filter is active show quick summary and compact list */}
        {ordersByDate.length > 0 && (
          <div style={{ marginBottom: 14, border: '1px solid #eee', padding: 12, borderRadius: 8, background: '#fafafa' }}>
            <div style={{ marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <strong>Orders on {selectedDate}:</strong>
              <span style={{ color: '#2c3e50' }}>{ordersByDate.length} order(s)</span>
            </div>
            <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
              {ordersByDate.map(o => (
                <li key={o._id} style={{ padding: '8px 0', borderTop: '1px solid #f0f0f0', fontSize: 13 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 600, color: '#2c3e50', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{o._id}</div>
                      <div style={{ color: '#666' }}>{o.userEmail || o.shippingDetails?.email || '—'} • Items: {(o.items && o.items.length) || 0}</div>
                    </div>
                    <div style={{ textAlign: 'right', minWidth: 140 }}>
                      <div style={{ color: '#111', fontWeight: 600 }}>{o.totalAmount ?? o.orderTotal ?? '—'}</div>
                      <div style={{ color: '#666', fontSize: 12 }}>{o.createdAt ? new Date(o.createdAt).toLocaleString() : '—'}</div>
                    </div>
                  </div>
                </li>
              ))}
